...
def backward(grad_fn, grad_of_outputs):
    ...
    if not type(gradients).__name__ == 'tuple':
        gradients = (gradients, )

    if not grad_fn.next_functions:
        return

    for grad, parent in zip(gradients, grad_fn.next_functions):
        if parent:
            backward(grad_fn = parent, grad_of_outputs = grad)
...
class Function:
    ...
    def apply(cls, *args):
        ...
        for pt in args:
            if type(pt).__name__ == 'Tensor':
                if not pt.grad_fn: # if parents don't have their own nodes.
                    pt.grad_fn = AccumulateGrad(pt)

                if not pt.requires_grad: # if parents do not need to be connected to the comp graph
                    backward_function.next_functions.append(None)
                else:
                    # Store information on the node, which links it to the comp graph
                    backward_function.next_functions.append(pt.grad_fn)
