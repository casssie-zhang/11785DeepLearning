...
class Tensor:
    ...
    def backward(self):
        ...
        # calculates the gradient w.r.t itself, fill with 1
        grad_outputs = Tensor(np.ones(self.data.shape))

        autograd_engine.backward(grad_fn = self.grad_fn, grad_of_outputs = grad_outputs)
